...
double bdss_proc_t::event_rates (double *rate, int n) const {
  int m = 0;
  double total = 0;
  RATE_CALC(params.la_nn * state.IN);
  RATE_CALC(params.la_ns * state.IN);
  RATE_CALC(params.la_sn * state.IS);
  RATE_CALC(params.la_ss * state.IS);
